...
const PERIODS: ReturnPeriod[] = [
  { label: "August 2025",   value: "08-2025", due: "11 Sep 2025", filed: false },
  { label: "July 2025",     value: "07-2025", due: "11 Aug 2025", filed: true  },
  { label: "June 2025",     value: "06-2025", due: "11 Jul 2025", filed: true  },
];
  ...
  const computeSummary = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await supabase
        .from("bills")
        .select("total, gst, gst_amt, subtotal")
      ...
      const taxableValue  = gstBills.reduce((s: number, b: any) => s + (b.subtotal ?? 0), 0);
      const totalGst      = gstBills.reduce((s: number, b: any) => s + (b.gst_amt ?? 0), 0);
      const cgst = totalGst / 2;
      const sgst = totalGst / 2;
      const igst = 0;
      const itc  = totalGst * 0.3; // Placeholder 30% ITC
      const netPayable = Math.max(0, totalGst - itc);

      setSummary({
        totalSales, taxableValue, cgst, sgst, igst,
        totalGst, itc, netPayable,
        b2bCount: Math.floor(gstBills.length * 0.6),
        b2cCount: Math.ceil(gstBills.length * 0.4),
      });
    } catch (e) {
      console.warn(e);
    }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: bg }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <View style={[styles.dueBox, { backgroundColor: period.filed ? "#D1FAE518" : "#FEF3C718", borderColor: period.filed ? "#10B98130" : "#F59E0B30" }]}>
          ...
          <Text style={{ fontSize: 13, fontFamily: "Inter_500Medium", color: period.filed ? "#065F46" : "#92400E" }}>
        ...
        {loading ? (
          ...
          <>
            ...
            <View style={{ gap: 12, marginTop: 8 }}>
              ...
              <TouchableOpacity style={[styles.actionBtn, { backgroundColor: card, borderWidth: 1, borderColor: dark ? "#2A2A3C" : "#E5E7EB" }]}>
