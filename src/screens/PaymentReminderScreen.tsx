...
import {
  ...
  ActivityIndicator, Alert, StatusBar, Platform, Switch, Linking,
} from "react-native";
...
export function PaymentReminderScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Load brand name for reminder messages
      const br = await loadBrand(user?.organisationId ?? null);
      if (br?.shopName) setBrandName(br.shopName);
    } catch (_) {}
    setLoading(false);
  }, [user]);
  ...
  const sendReminder = (bill: Bill, channel: "whatsapp" | "sms") => {
    const msg = `Dear ${bill.customer}, your payment of \u20b9${bill.total.toLocaleString("en-IN")} is due${bill.due_date ? ` on ${new Date(bill.due_date).toLocaleDateString("en-IN")}` : ""}. Kindly clear the dues. \u2013 ${brandName}`;
    ...
    if (channel === "whatsapp") {
      ...
      const url = `whatsapp://send?phone=91${phone}&text=${encoded}`;
      Linking.openURL(url).catch(() =>
        Alert.alert("WhatsApp not installed", "Could not open WhatsApp. Please install it.")
      );
    } else {
      const phone = (bill.phone || "").replace(/\D/g, "");
      if (!phone) {
